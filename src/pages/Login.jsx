...
import { BookOpen, Sparkles, LayoutDashboard, ShieldCheck } from 'lucide-react';
import toast from 'react-hot-toast';
import { login } from '../api/apiClient';
import { getRedirectPathForRole, normalizeUserRole, setAuth } from '../hooks/useAuth';
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.role && form.role !== resolvedRole) {
        toast.error(`This account belongs to ${resolvedRole}.`);
        return;
      }

      setAuth({ id: res.userId, username: res.username, role: resolvedRole }, res.token);
      ...
      <div className="flex w-full flex-col justify-center px-8 sm:px-16 lg:w-1/2 lg:px-24 xl:px-32 relative z-10">
        <div className="mx-auto w-full max-w-md">
          ...
          <p className="text-sm text-slate-400 mb-8">
            Login as Admin or Student to enter the correct workspace.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-5 rounded-2xl bg-white/[0.03] p-6 border border-white/[0.08] shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] backdrop-blur-xl">
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-200">Select Role</label>
                <select
                  name="role"
                  value={form.role}
                  onChange={handleChange}
                  className="w-full rounded-xl border border-white/10 bg-[#12131c] px-4 py-3 text-white transition-all focus:border-indigo-500 focus:bg-[#1a1c29] focus:outline-none focus:ring-1 focus:ring-indigo-500"
                >
              ...
              <div className="rounded-2xl border border-indigo-500/20 bg-indigo-500/10 p-4 text-sm text-indigo-100">
                ...
                <div className="mt-3 space-y-1">
                  <p><span className="font-medium">Admin:</span> <code>admin@gmail.com</code> / <code>admin123</code></p>
